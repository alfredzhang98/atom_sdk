...
class MysqlBaseHandler:
    def __init__(self, host, port, user, password, database):
        ...
        if not host or not port or not user or not password or not database:
            raise KeyError ("None input for datebase config")
    ...
    def _connect(self):
        try:
            ...
            with temp_connection.cursor() as temp_cursor:
                ...
                version = temp_cursor.fetchone()[0]
            temp_connection.close()
            # Setting related configs for different version
            charset = 'utf8mb4' if version.startswith('8.') else 'utf8'

            if not self.database_exists():
                self.create_database()
                self.logger.info(f"Success create the database")
    ...
    def close_connection(self):
        ...
        try:
            ...
            self.logger.error(f"Error in close connection: {e}")
...
class MysqlTableHandler(MysqlBaseHandler):
